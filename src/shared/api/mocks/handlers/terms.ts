...
const termsDetailTable: Record<TermCode, TermsDetail> = {
  service_terms: {
    code: "service_terms",
    title: "서비스 이용약관 동의",
    summary:
...
export const termsHandlers = [
  http.get("/api/v1/terms", () => {
    const sorted = [...termsAgreementTable].sort(
  ...
  http.get("/api/v1/terms/:code", ({ params }) => {
    const code = String(params.code) as TermCode;
    const item = termsDetailTable[code] ?? null;
