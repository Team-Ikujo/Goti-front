...
const writeStoredSocialVerifyToken = (socialVerifyToken: string | null) => {
   ...
   if (socialVerifyToken) {
      window.sessionStorage.setItem(SOCIAL_VERIFY_TOKEN_STORAGE_KEY, socialVerifyToken);
      return;
   }
...
export type AuthState = {
   ...
   setAuthTokens: (payload: { accessToken: string | null; socialVerifyToken: string | null }) => void;
...
export const useAuthStore = create<AuthState>()(
   persist(
      (set, get) => ({
         ...
         setRecentLoginProvider: recentLoginProvider => set({ recentLoginProvider }),
      ...
      }),
      {
         ...
         merge: (persistedState, currentState) => {
            ...
            return {
               ...currentState,
               recentLoginProvider: persisted?.recentLoginProvider ?? currentState.recentLoginProvider,
               currentUserId: persisted?.currentUserId ?? currentState.currentUserId,
               socialVerifyToken: socialVerifyToken ?? currentState.socialVerifyToken,
               isManualLogout: persisted?.isManualLogout ?? currentState.isManualLogout,
            };
         },
         partialize: state => ({
