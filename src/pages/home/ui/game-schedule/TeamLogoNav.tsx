...
function TeamLogoNav({ onNavigateTeam }: TeamLogoNavProps) {
   return (
      <div className="flex w-full">
         {teamOrder.filter(team => team === '삼성' || team === 'KIA').map(team => {
            ...
            return (
               <button
                  ...
                  disabled={!isEnabled}
                  className={cn(
                     'aspect-square min-h-14 max-h-25 min-w-15] w-full p-1.25 lg:p-2.5 flex items-center justify-center overflow-hidden transition-colors rounded-none lg:rounded-xl',
